...
export default function DMButton({ onClick, hasNotification }) {
  ...
  return (
    ...
    >
      ...
      {hasNotification && (
        <span className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold border-2 border-white animate-pulse">!</span>
      )}
    </button>
  );
}

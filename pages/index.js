  ...
  return (
    <div className="min-h-screen bg-[var(--color-background)] text-[var(--color-text-primary)] transition-colors duration-500 flex flex-col items-center relative overflow-hidden select-none">
      ...
      <header className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex justify-between items-center z-20">
        <div
          className="flex items-center gap-3 cursor-pointer text-[var(--color-text-primary)] animate-scale-in"
      ...
      <main className="text-center relative z-10 flex flex-col items-center justify-center min-h-[60vh] px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl">
          <h1 className="text-5xl sm:text-6xl md:text-7xl font-extrabold mb-6 text-[var(--color-text-primary)] leading-tight">
            {['Your', 'second', 'brain', 'for', 'the', 'AI', 'era'].map((word, i) => (
              <span
                key={i}
          ...
          <p className="text-lg sm:text-xl md:text-2xl max-w-xl mx-auto mb-10 text-[var(--color-text-secondary)] animate-fade-in-up" style={{ animationDelay: '0.8s' }}>
          ...
          <div className="flex justify-center mt-10">
            <Link href="/auth">
              <button
                className="px-8 py-4 rounded-full bg-[var(--color-brand)] text-white text-xl font-bold shadow-lg hover:bg-[var(--color-brand-hover)] transition-colors focus:outline-none focus:ring-4 focus:ring-[var(--color-brand)] focus:ring-opacity-50 animate-scale-in"
                style={{ minWidth: '200px', animationDelay: '1s' }}
              >
                Start Now
              </button>
            </Link>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-5xl mx-auto mt-16 sm:mt-20 md:mt-24">
          {features.map((feature, i) => {
            ...
            return (
              <div
                key={feature.title}
                className={`p-8 rounded-2xl border border-[var(--color-border)] animate-scale-in-up feature-card shadow-lg ${textPrimary}`}
                style={{
                  animationDelay: `${0.2 * (i + 1) + 1.2}s`,
                  background: isDark
                    ? 'rgba(30, 41, 59, 0.85)'
              ...
              >
                <div className="flex items-center justify-center w-14 h-14 bg-[var(--color-brand-muted)] rounded-lg mb-5 text-[var(--color-brand)] animate-fade-in" style={{ animationDelay: `${0.2 * (i + 1) + 1.3}s` }}>
                  {feature.icon}
                </div>
                <h3 className={`text-xl font-semibold mb-2 ${textPrimary} animate-fade-in-up`} style={{ animationDelay: `${0.2 * (i + 1) + 1.4}s` }}>{feature.title}</h3>
                <p className={`text-base ${textSecondary} animate-fade-in-up`} style={{ animationDelay: `${0.2 * (i + 1) + 1.5}s` }}>{feature.description}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="max-w-4xl mx-auto mt-20 mb-8">
          <div className="flex flex-col md:flex-row gap-8 items-center justify-center">
            {[
              {
                quote: "Noteify is the first note app that feels truly smart and delightful to use.",
                author: "Alex P.",
                role: "Product Designer",
                img: "/file.svg"
              },
              {
                quote: "The AI features are a game changer. I never lose track of my ideas now.",
                author: "Jamie L.",
              ...
              <div
                key={i}
                className={`rounded-2xl shadow-lg border border-[var(--color-border)] p-6 flex-1 min-w-[260px] text-[var(--color-text-primary)] animate-scale-in-up`}
                style={{
                  animationDelay: `${0.2 * (i + 1) + 1.8}s`,
                  background: ['sunset', 'forest', 'dark'].includes(theme)
                    ? 'rgba(30, 41, 59, 0.85)'
                    : 'rgba(255,255,255,0.8)'
                }}
              >
                <p className={`text-lg font-medium mb-3 text-[var(--color-text-primary)] animate-fade-in-up`} style={{ animationDelay: `${0.2 * (i + 1) + 1.9}s` }}>{testimonial.quote}</p>
                <div className="flex items-center gap-3 animate-fade-in" style={{ animationDelay: `${0.2 * (i + 1) + 2}s` }}>
                  <img src={testimonial.img} alt="User" className="w-10 h-10 rounded-full bg-[var(--color-bg-subtle)]" />
                  <span className={`font-semibold text-[var(--color-text-primary)]`}>{testimonial.author}</span>
                  <span className={`text-xs text-[var(--color-text-secondary)]`}>{testimonial.role}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <footer className="w-full text-center p-6 text-base text-[var(--color-text-secondary)] z-10 mt-auto animate-fade-in-up" style={{ animationDelay: '2.4s' }}>
      ...
      <style jsx global>{`
        ...
        .feature-card:hover {
          transform: perspective(1000px) rotateX(-2deg) rotateY(-2deg) scale3d(1.05, 1.05, 1.05);
        }
      `}</style>
    </div>
  );

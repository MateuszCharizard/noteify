  ...
  return (
    <div className={`${theme === 'dark' ? 'dark' : ''} min-h-screen select-none ${theme === 'dark' ? 'bg-black text-gray-100' : 'bg-white text-gray-900'} transition-all duration-500 flex flex-col items-center relative overflow-hidden`}>
      ...
      <div className="absolute top-4 right-4 flex items-center gap-4 z-20">
        ...
        <Link href="/profile" className="w-10 h-10 rounded-full bg-gray-200 dark:bg-gray-800 transition-all duration-300 hover:scale-110 cursor-pointer z-20 overflow-hidden">
            <img src={profile?.avatar_url || `https://placehold.co/40x40/e2e8f0/4a5568?text=${profile?.username?.charAt(0) || 'A'}`} alt="Profile" className="w-full h-full object-cover" />
        </Link>
      </div>
      ...
      <header className="fixed top-0 left-0 right-0 flex justify-center pt-4 z-10">
        <img
          ...
          alt="Noteify Logo"
          className={`w-24 h-24 cursor-pointer animate-bounce-slow hover:animate-spin-once ${theme === 'dark' ? 'invert' : ''}`}
